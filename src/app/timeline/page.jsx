...
const TimelinePage = () => {
  ...
  const timelineCard = message.map((theMan, ind) => (
    <div
      key={ind}
      className="bg-base-300 rounded-box flex gap-4 my-4 shadow-md h-20 p-4"
    >
      {" "}
      ...
      <div>
        ...
        <p>
          {" "}
          {months[theMan.month - 1]} {theMan.day}, {theMan.year}{" "}
        </p>
      </div>
    </div>
  ));
  ...
  const call = message.filter((call) => call.btnClicked === "Call");
  ...
  const text = message.filter((text) => text.btnClicked === "Text");
  ...
  const video = message.filter((video) => video.btnClicked === "Video");
